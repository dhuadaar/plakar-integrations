...
# of the backblaze_s3 module. The two copies are expected to drift slightly
# over time (e.g. connector-specific location URIs) - that's fine, keep them
# independent rather than re-introducing a shared file.
#
...
#
# The native connector accepts two location URL forms (see schema.json):
#   native (default): b2://<bucket>/<optional-prefix>
#   legacy:            b2://<endpoint>/<bucket>/<optional-prefix>
# Select which one this run exercises with B2_LOCATION_FORM=native|legacy
# (env var or set in b2-creds.env). Both forms resolve to the exact same
# bucket/prefix and B2 native API calls - "endpoint" is only used to decide
# how to split the URL path, it is never dialed. Default: native.
...
# environment (e.g. `B2_PREFIX= ./test-e2e.sh` to force a fresh run) - the
# creds file below is meant to supply defaults, not clobber explicit
# invocation-time overrides.
for _v in B2_PREFIX B2_LOCATION_FORM B2_LEGACY_ENDPOINT; do
	if [ "${!_v+set}" = "set" ]; then
		eval "_caller_${_v}=\${$_v}"
	fi
done
...
unset _v _caller_var _caller_B2_PREFIX _caller_B2_LOCATION_FORM _caller_B2_LEGACY_ENDPOINT
...
B2_LOCATION_FORM="${B2_LOCATION_FORM:-native}"
B2_LEGACY_ENDPOINT="${B2_LEGACY_ENDPOINT:-s3.us-west-004.backblazeb2.com}"
...
case "$B2_LOCATION_FORM" in
native)
	LOCATION="${LOCATION_SCHEME}://${B2_BUCKET}/${B2_PREFIX}"
	;;
legacy)
	LOCATION="${LOCATION_SCHEME}://${B2_LEGACY_ENDPOINT}/${B2_BUCKET}/${B2_PREFIX}"
	;;
*)
	echo "!! unknown B2_LOCATION_FORM: $B2_LOCATION_FORM (expected 'native' or 'legacy')" >&2
	exit 1
	;;
esac
echo "==> Location form: $B2_LOCATION_FORM"
